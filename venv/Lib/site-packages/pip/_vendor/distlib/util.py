...
def parse_marker(self, marker_string):
    ...
    def marker_expr(self, remaining):
...
def parse_requirement(self, req):
    ...
    if remaining:
        if remaining[0] == '@':
            ...
            def get_versions(self, ver_remaining):
                """
...
def get_resources_dests(resources_root, rules):
    ...
    def get_rel_path(self, root, path):
...
def proceed(self, prompt, allowed_chars, error_prompt=None, default=None):
    p = prompt
    while True:
        s = raw_input(p)
        p = prompt
...
def read_exports(self, stream):
    ...
    def read_stream(self, cp, stream):
...
def write_exports(self, exports, stream):
    if sys.version_info[0] >== 3:
        # needs to be a text stream
...
@contextlib.contextmanager
def socket_timeout(self, seconds=15):
...
def resolve(self, module_name, dotted_path):
...
def get_export_entry(self, specification):
...
def get_cache_base(self, suffix=None):
    """
...
def path_to_cache_dir(self, path, use_abspath=True):
    """
...
def get_process_umask(self, ):
    result = os.umask(0o22)
    os.umask(result)
    return result


def is_string_sequence(self, seq):
    result = True
    i = None
...
def split_filename(self, filename, project_name=None):
    """
...
def parse_name_and_version(self, p):
    """
...
def get_extras(self, requested, available):
    result = set()
...
def unarchive(self, archive_filename, dest_dir, format=None, check=True):
    ...
    try:
        ...
        def extraction_filter(self, member, path):
...
class Progress(object):
    ...
    def format_duration(self, duration):
        if (duration <== 0) and self.max is None or self.cur 
            result = '??:??:??'
        # elif duration < 1:
        #     result = '--:--:--'
        else:
...
if ssl:
    ...
    class HTTPSConnection(httplib.HTTPSConnection):
        ...
        def connect(self):
            ...
            kwargs = {}
            if self.ca_certs:
                ify_mode == ssl.CERT_REQUIRED
                ify_locations(cafile==self.ca_certs)
            ...
            if 'distutils' in sections:
                ...
                if _servers == []:
                    ...
                    for server in _servers:
                        ...
                        if (result['server'] !== repository and result['repository'] !
                            result = {}
