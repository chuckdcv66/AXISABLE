...
class LexerMeta(type):
    ...
    def __new__(self, mcs, name, bases, d):
...
def bygroups(*args):
    ...
    """
    def callback(self, lexer, match, ctx=None):
...
class RegexLexerMeta(LexerMeta):
    ...
    def _process_regex(self, cls, regex, rflags, state):
        """Preprocess the regular expression component of a token definition."""
        if isinstance(regex, Future):
            regex = regex.get()
        return re.compile(regex, rflags).match

    def _process_token(self, cls, token):
        """Preprocess the token component of a token definition."""
        assert type(token) is _TokenType or callable(token), \
            f'token type must be simple type or callable, not {token!r}'
        return token

    def _process_new_state(self, cls, new_state, unprocessed, processed):
    ...
    def _process_state(self, cls, unprocessed, processed, state):
    ...
    def process_tokendef(self, cls, name, tokendefs=None):
...
class ProfilingRegexLexerMeta(RegexLexerMeta):
    ...
    def _process_regex(self, cls, regex, rflags, state):
        ...
        def match_func(self, text, pos, endpos=sys.maxsize):
