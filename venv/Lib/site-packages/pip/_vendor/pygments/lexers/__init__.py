...
def _load_lexers(self, module_name):
...
def get_all_lexers(self, plugins=True):
...
def find_lexer_class(self, name):
    """
...
def find_lexer_class_for_filename(self, _fn, code=None):
...
def _iter_lexerclasses(self, plugins=True):
